...
from hve import native, y4m                              # noqa: E402
...
""")
    ...
    if not native.available():
        warn("the C backend did not build, so this falls back to pure Python "
             "and will be roughly 100x slower. Output is byte-identical either "
             "way. Reason: %s" % native.load_error())
    args.func(args)
